...
describe('pagination', () => {
  it('walks every page and stops once Total is reached', async () => {
    const { fetch, requests } = stubFetch((url) => {
      if (!isList(url)) return jsonResponse({ ID: url.searchParams.get('ID') })
      const page = Number(url.searchParams.get('Page'))
      return page === 1 ? listPage(['a', 'b'], 3) : listPage(['c'], 3)
    })

    const { written, writer } = collectWrites()
    const summary = await runFetch(testClient(fetch), writer, baseOptions({ pageSize: 2 }))
  ...
  it('fetches an order only once when page drift lists it twice', async () => {
    ...
    const { fetch, requests } = stubFetch((url) => {
      if (!isList(url)) return jsonResponse({ ID: url.searchParams.get('ID') })
      const page = Number(url.searchParams.get('Page'))
      return page === 1 ? listPage(['a', 'b'], 4) : listPage(['b', 'c'], 4)
    })
...
describe('request construction', () => {
  it('percent-encodes search terms instead of splicing them into the query', async () => {
    const { fetch, requests } = stubFetch((url) =>
      isList(url) ? listPage([], 0) : jsonResponse({}),
    )

    await runFetch(
      testClient(fetch),
      collectWrites().writer,
      baseOptions({ search: 'widgets & bolts #2', status: 'AUTHORISED' }),
    )
  ...
  it('sends the date filter as an unambiguous UTC instant', async () => {
    ...
    await runFetch(
      testClient(fetch),
      collectWrites().writer,
      baseOptions({ dateFilter: 'UpdatedSince' }),
    )
  ...
  it('never sends CreatedSince, which /purchaseList does not support', async () => {
    ...
    await runFetch(testClient(fetch), collectWrites().writer, baseOptions())
...
describe('resilience', () => {
  it('retries a 429 and honours Retry-After', async () => {
    ...
    const { fetch } = stubFetch((url) => {
      if (!isList(url)) return jsonResponse({ ID: 'a' })
      listCalls++
      if (listCalls === 1) {
    ...
    const summary = await runFetch(testClient(fetch), collectWrites().writer, baseOptions())
  ...
  it('does not retry a client error', async () => {
    ...
    await expect(
      runFetch(testClient(fetch), collectWrites().writer, baseOptions()),
    ).rejects.toThrow(/401/)
  ...
  it('reports a failed detail fetch without abandoning the rest of the run', async () => {
    const { fetch } = stubFetch((url) => {
      if (isList(url)) return listPage(['good', 'bad'], 2)
    ...
    const { written, writer } = collectWrites()
    const summary = await runFetch(testClient(fetch, { maxRetries: 0 }), writer, baseOptions())
...
describe('validation', () => {
  ...
  it('writes the detail payload byte for byte as it arrived', async () => {
    ...
    const { fetch } = stubFetch((url) =>
      isList(url) ? listPage(['a'], 1) : new Response(raw, { status: 200 }),
    )

    const { written, writer } = collectWrites()
    await runFetch(testClient(fetch), writer, baseOptions())
  ...
  it('records a failure when a detail body is not JSON at all', async () => {
    const { fetch } = stubFetch((url) =>
      isList(url) ? listPage(['a'], 1) : new Response('<html>maintenance</html>', { status: 200 }),
    )
...
describe('concurrency', () => {
  it('keeps no more than maxConcurrency detail requests in flight', async () => {
    ...
    const { fetch } = stubFetch(async (url) => {
      if (isList(url)) return listPage(Array.from({ length: 12 }, (_, i) => `PO-${i}`), 12)
    ...
    const summary = await runFetch(
      testClient(fetch),
      collectWrites().writer,
      baseOptions({ pageSize: 20, maxConcurrency: 3 }),
    )
  ...
  it('still fetches everything when maxConcurrency is 1', async () => {
    ...
    const { fetch } = stubFetch(async (url) => {
      if (isList(url)) return listPage(['a', 'b', 'c'], 3)
      inFlight++
      peak = Math.max(peak, inFlight)
    ...
    const { written, writer } = collectWrites()
    await runFetch(testClient(fetch), writer, baseOptions({ pageSize: 10, maxConcurrency: 1 }))
...
describe('cancellation', () => {
  it('stops making detail requests once the caller cancels', async () => {
    ...
    const { fetch } = stubFetch(async (url) => {
      if (isList(url)) return listPage(Array.from({ length: 40 }, (_, i) => `PO-${i}`), 40)
    ...
    const summary = await runFetch(
      testClient(fetch),
      collectWrites().writer,
      baseOptions({ pageSize: 50, maxConcurrency: 2, signal: controller.signal }),
    )
  ...
  it('refuses to request the next page after the signal aborts', async () => {
    ...
    const { fetch } = stubFetch((url) => {
      ...
      return listPage(['a', 'b'], 4)
    })

    const { written, writer } = collectWrites()
    await expect(
      runFetch(testClient(fetch), writer, baseOptions({ pageSize: 2, signal: controller.signal })),
...
describe('failure reporting', () => {
  it('stops storing failure details past the cap so an outage cannot fill memory', async () => {
    const { fetch } = stubFetch((url) => {
      if (isList(url)) return listPage(Array.from({ length: 105 }, (_, i) => `PO-${i}`), 105)
  ...
  it('does not write a file for an order that could not be fetched', async () => {
    const { fetch } = stubFetch((url) =>
      isList(url) ? listPage(['bad'], 1) : new Response('boom', { status: 500 }),
    )

    const { written, writer } = collectWrites()
    await runFetch(testClient(fetch, { maxRetries: 0 }), writer, baseOptions())
...
describe('the PurchaseOrderFetcher class', () => {
  ...
  it('accepts any object with the two ApiReader methods, not just DearAPIClient', async () => {
    ...
    const reader: ApiReader = {
      async get<T>(_options: RequestOptions, read: (body: unknown) => T): Promise<T> {
    ...
    const { written, writer } = collectWrites()
    const summary = await runFetch(reader, writer, baseOptions())
