...
const passThrough = (body: JsonValue) => body;
...
describe('cancellation and timeouts', () => {
  it('gives up on a request that outlives the timeout', async () => {
    ...
    const fetchFunction: typeof fetch = (_input, init) =>
    ...
    await expect(
      new DearApiClient(config, fetchFunction, silentLogger).get(LIST, passThrough),
    ).rejects.toThrow();
  ...
  it('stops immediately when the caller cancels, without spending a retry', async () => {
    ...
    const client = new DearApiClient(testConfig({ maxRetries: 5 }), fetchFunction, silentLogger);
